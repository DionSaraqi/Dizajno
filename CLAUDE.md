...
- **GLB model check on upload**: picking a GLB parses it client-side first (`utils/glbAnalysis.ts` — GLTFLoader + Draco decoder pinned to drei's default 1.5.5; parse failure blocks the upload since the designer would fail too), measures its bounding box / triangle count / named materials, and shows live warnings against the typed W/D/H (`utils/glbChecks.ts`, pure + unit-tested: proportion mismatch vs the designer's uniform min-ratio scaling, cm/mm/inch unit detection, 150k-triangle budget, 50 MB server cap, implausible-dimension range). Buttons: "Apply measured dimensions" (`measuredDimsForApply` — unit-converts or anchors on typed height), "Add material slots from model", and a 3D preview (`GlbPreview.tsx`, dynamic ssr:false) rendering the model through the designer's own `GLTFModel` inside a wireframe of the typed-dims box — gaps = hitbox dead space. "Measure current model" re-checks an already-attached asset (requires CORS on the R2 public host).
