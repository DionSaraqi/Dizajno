...
| 7b — Supplier portal backend | ✓ Done | Six new controllers under `/api/supplier/*`: products (CRUD + Draft↔Pending↔Published↔Hidden↔Removed transitions; trusted suppliers auto-publish, untrusted go to Pending picked up by Phase 7a's queue), variants (CRUD + GLB/SVG asset attachment with cross-supplier guard + scene-references-protected delete), textures (library CRUD + full-replacement per-variant slot bindings), categories (supplier suggests → Pending → admin moderates), members (Owner-only CRUD with last-Owner protection on demote and remove), profile (Owner-only edit, slug immutable). `SupplierAssetsController` now accepts `Glb` + `SvgPreview` kinds. `SupplierMembershipExtensions` gained `IsActiveMemberOf` + `IsActiveOwnerOf` helpers. Migration `0010_AssetOwnerNonUnique` fixes a Phase-1 schema bug where `assets.owner_supplier_id` was UNIQUE (EF auto-paired `Asset.OwnerSupplier` ↔ `Supplier.LogoAsset` as 1:1 off the matching single navs) — `Supplier.OwnedAssets` collection + explicit `HasMany().WithOne()` disambiguates. 21 new integration tests (120 total green). |
| 7c — Supplier portal UI | ✓ Done | `/supplier/[supplierId]/{products,products/new,products/[id],textures,members,profile}` Next.js routes on top of the Phase 7b endpoints. Product list with status filter + lifecycle buttons; product editor with tabbed Info + Variants (collision-box rows, material-slot rows, GLB + SVG upload through the three-step presign helper, texture-slot binder); texture library with image upload; Owner-only member management (roster + role change + remove + Owner-issued tokenized invite link with one-time clipboard copy); Owner-only profile editor with logo upload. Inline category-suggest from the product create form. New backend endpoint `POST /api/supplier/invites` (Owner-gated, mirrors the admin one) so Owners can grow their team without admin involvement. `CategoryDto` extended with `id` so the public catalog can drive the product create form's category picker. Projects nav now shows "Supplier portal" instead of "Inbox" for users with memberships. 2 new integration tests (122 total green). |
...
### Phase 7b — Supplier portal backend ✓ Done

Branch: `feat/backend-foundation`. Built on top of Phase 7a.

**Goal:** suppliers can create + edit their own catalog without admin gatekeeping. Untrusted suppliers' new products feed Phase 7a's moderation queue; trusted suppliers' products auto-publish. Phase 7c will plug the UI on top of the same endpoint surface.

Built:

- **Migration `0010_AssetOwnerNonUnique`.** Pre-existing schema bug from Phase 1: EF auto-paired `Asset.OwnerSupplier` (single nav) with `Supplier.LogoAsset` (single nav) into a 1:1 relationship, which made `ix_assets_owner_supplier_id` UNIQUE — every supplier was capped at owning exactly one asset. Fix: add `Supplier.OwnedAssets` collection navigation + `AssetConfiguration.HasOne(x => x.OwnerSupplier).WithMany(s => s.OwnedAssets)`, then the auto-generated migration drops + recreates the index as non-unique. Surfaced because Phase 7b tests routinely upload multiple Image assets per supplier (texture library) — never hit in earlier phases.
- **`SupplierAssetsController`** extended to accept `AssetKind.Glb` + `AssetKind.SvgPreview`. `AssetUploadRules` already defined the size caps + MIME allow-lists; just the gate at the top of the controller needed widening.
- **`SupplierProductsController`** (`/api/supplier/products`). List (filterable by `supplierId` / `status` / search), get-by-id, create (lands in `Draft`, validates category family + Approved status, slug uniqueness 409), update (immutable family + slug; category change must stay in same family + be Approved), `POST /{id}/publish` (Draft → `Pending` for untrusted suppliers or `Published` for trusted; Hidden → `Published` directly, never re-enters Pending — already moderated once), `POST /{id}/hide`, `POST /{id}/remove` (terminal; historical `QuoteLine.variant_snapshot` already snapshots so old quotes survive). Publish refuses 409 if the product has zero variants. Every transition writes an `audit_log` row.
- **`SupplierVariantsController`** (nested under products + flat for variants). Create checks SKU uniqueness, auto-increments `SortOrder` if not supplied. Update mirrors create. Delete refuses 409 if any `PlacedItem` / `Opening` / `Wall.paint_product_variant_id` / `Floor.flooring_product_variant_id` references the variant — the supplier must hide the product instead so existing project scenes keep rendering. `POST /attach-glb` + `POST /attach-preview` validate the asset's `OwnerSupplierId` matches the variant's supplier and the `AssetKind` matches; detach is the same endpoint with `assetId = Guid.Empty`.
- **`SupplierTexturesController`** (`/api/supplier/textures` + `/api/supplier/variants/{id}/texture-slots`). Library CRUD with per-supplier `(supplierId, name)` uniqueness; delete 409s if any slot binding still references the texture (FK is RESTRICT). The slot-binding `PUT` is a full replacement (delete-all + insert-all in a transaction) — matches the supplier-edits-the-whole-table UX better than a diff. Cross-supplier texture references 400 before reaching the existing `trg_pv_texture_slot_supplier_match` Postgres trigger. At most one `IsDefault` per (variant, slotName) is enforced by trimming conflicts to the first row.
- **`SupplierCategoriesController`** (`/api/supplier/categories`). List supplier's own suggestions (Pending + Approved). Suggest endpoint creates a `CategoryStatus.Pending` row tagged with `SuggestedBySupplierId`, slugifies the name (`Pendant Lights` → `pendant-lights`), validates parent category is in the same family + already Approved, 409s on family-scoped slug collisions. Phase 7a's moderation queue picks up the row.
- **`SupplierMembersController`** (`/api/supplier/members`, Owner-only mutations). List visible to Owners + Staff. Role change + remove require `IsActiveOwnerOf(supplierId)`. **Last-Owner protection**: demoting the last Owner to Staff, or removing the last Owner, returns 409 with a directive to promote another member first.
- **`SupplierProfileController`** (`/api/supplier/profile/{supplierId}`, Owner-only edit). Slug is immutable (URL stability + audit-log entity-id semantics). `IsTrusted` + `SuspendedAt` stay admin-only. Logo asset must be `AssetKind.Image` owned by the same supplier.
- **`SupplierMembershipExtensions`** gained `IsActiveMemberOf(supplierId)` + `IsActiveOwnerOf(supplierId)` — single-supplier yes/no checks reused across all six new controllers.
- **Tests.** `SupplierPortalTests`: 21 new integration tests covering Draft start, foreign-supplier 403, duplicate slug 409, no-variant publish 409, trusted-supplier auto-publish, untrusted-supplier → Pending lands in admin queue, Hidden→Published never re-triggers Pending even when the supplier was untrusted in the meantime, Removed terminal state, suspended supplier blocks portal access, variant CRUD + cross-supplier GLB rejection, variant delete blocked when referenced by scene, texture create + delete + lifecycle, slot binding cross-supplier rejection, slot binding full-replace replaces existing rows, category suggest lands in admin queue, last-Owner demote/remove 409, Staff cannot change roles (403), Owner can edit profile / Staff cannot, presign accepts `Glb` + `SvgPreview` and rejects `CadSource`. Full suite is **120 tests** across **9 classes**.

Out of scope this phase (lands in 7c):

- **Supplier portal UI** — `/supplier/[supplierId]/*` pages on top of these endpoints. Product editor with variant tab + collision-box numeric form + GLB drag-and-drop upload + texture library. Member management + profile pages for Owners.
- **Lighting + appliance GLB seed data** — easier once real suppliers can upload via the portal in 7b.

### Phase 7c — Supplier portal UI ✓ Done

Branch: `feat/backend-foundation`. Built on top of Phase 7b.

**Goal:** the portal is now self-serve from the browser. Suppliers manage their catalog without dropping into Swagger. Owners can also invite Staff without going through admin — a small Phase-7c backend addition (`POST /api/supplier/invites`) mirrors the admin endpoint but is Owner-gated.

Built:

- **`SupplierInvitesController`** — Owner-side mirror of `AdminInvitesController` at `/api/supplier/invites`. Same tokenized flow (SHA-256 hash at rest, raw token + `acceptUrl` returned once on create). Read is open to any active member; create + revoke require Owner. Audit log entries use `supplier_invite.create_by_owner` and `supplier_invite.revoke_by_owner` so the admin can distinguish admin-issued from supplier-issued invites in the trail. The existing public accept flow at `/api/invites/{token}/accept` covers both invite sources unchanged.
- **`CategoryDto.id`** — the public catalog DTO now carries the category id alongside slug/name/family. Lets the product create form drive its category picker by id instead of by string name. Additive, no migration.
- **Frontend portal shell.** `/supplier` (picker that auto-redirects single-supplier users), `/supplier/[supplierId]/layout.tsx` (tabbed nav with Owner-only Members + Profile entries; suspended members hit a polite block screen). `/supplier/[supplierId]` redirects to `/products`.
- **Products UI.** List with status filter (Draft/Pending/Published/Hidden) + search + per-row lifecycle buttons (publish/hide/remove with safety confirms). Create form at `/products/new` with family-aware fields (BuildingMaterial + Liter → Coverage rate, BuildingMaterial → Texture URL hint, etc.) and an inline "Suggest a new category" modal that fires off `POST /api/supplier/categories`. Product editor at `/products/[productId]` with tabbed Info + Variants:
  - **Info tab** edits everything except family + slug; supports raw SVG preview markup and free-form attributes JSON.
  - **Variants tab** renders one card per variant. Each has a dimensions/price/color edit, a collision-box numeric editor (rows of offsetX/offsetZ/width/depth — defaults to empty for rectangular furniture per the Phase 7 planning decision), a material-slot defaults editor (rows of slot name + hex color), GLB upload + SVG-preview upload via the three-step presign+PUT+finalize flow, and a per-variant texture-slot binder that picks from the supplier's library. Delete refuses gracefully with the backend's 409 message when scenes still reference the variant.
- **Textures UI.** Card grid with image preview, name, tags, repeat counts. Create modal uploads a JPEG/PNG, names + tags it, sets repeat U/V (1–32). Delete is blocked when any variant slot still binds the texture (409 from backend surfaced as a tooltip).
- **Members UI.** Roster with per-row role picker (Owner/Staff) + remove button (Owner-only). Self-remove redirects back to `/supplier`. Invite flow uses the new Owner endpoint; modal shows the one-time `acceptUrl` with a clipboard-copy button + flips into a success state. Staff see the page in read-only mode with a banner explaining their actions will 403.
- **Profile UI.** Owner-only edit screen with logo upload (Image asset via supplier presign). Staff URL-hackers bounce to `/products`. Shows IsTrusted / Suspended status pills as read-only indicators.
- **Upload helper.** `lib/api.ts` got `uploadSupplierFile(supplierId, file, kind)` — wraps the three-step presign → PUT → finalize flow into one call. Surfaces R2-misconfigured errors as plain `Error` messages so the UI can alert the supplier without crashing. Used by all four file-upload entry points (GLB, SVG preview, texture image, supplier logo).
- **Projects nav** now surfaces a green "Supplier portal" button (replacing the old "Inbox" link) for users with at least one membership. Routes to `/supplier` which auto-redirects single-supplier users.
- **Tests.** Two new `Invites_*` tests in `SupplierPortalTests` covering Owner-side invite gating + foreign-supplier 403 + token-only-on-create semantics. Full suite is **122 tests** across **9 classes**.

Out of scope this phase (parked):

- **Visual collision-box editor** — still the numeric form, per Open Question #10.
- **Drag-and-drop variant reordering** — sort order is editable but only via a numeric input.
- **Bulk variant operations** (clone, copy dimensions from another variant) — not in v1 scope.
- **Image cropping / thumbnail generation in the texture upload** — texture rows accept a separate `thumbnailAssetId` on the backend but the UI only uses the main asset for previews.
