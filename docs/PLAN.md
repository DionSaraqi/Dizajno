...
| 7c — Supplier portal UI | pending | Frontend portal at `/supplier/[supplierId]/{products,textures,members,profile}`: product list + editor (variant tab with material-slot/collision-box numeric forms, GLB drag-and-drop upload, texture library tab), category-suggest flow, member management UI, profile editor. Existing `/supplier/quotes` inbox stays as-is. |
