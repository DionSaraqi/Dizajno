...
- **Pricing pass + showcase tier (post-Phase 6 polish).** All 12 Phase-1 furniture rows + the 4 Phase-6 rows + 3 new material variants got concrete `BasePrice` values so the request-quote dialog renders real subtotals. New `DataSeeder.BackfillVariantPricesAsync` step updates null prices on existing dev DBs without requiring a `docker compose down -v`. Three new building-material tiers (`premium-eco-paint`, `exterior-weather-paint`, `budget-vinyl-flooring`, `engineered-hardwood`) showcase the calculator with distinct per-unit prices: paint coverage rates of 8/10/12 m²/L produce visibly different per-line totals; flooring tiers span €11 → €45/m². The `RequestQuoteDialog` Materials & finishes section now shows per-unit price next to each row, a line subtotal (`= €126`) once the row is selected, and a Materials subtotal footer summing all selected materials. Materials contributions also fold into the per-supplier subtotal preview at the top of the dialog.
